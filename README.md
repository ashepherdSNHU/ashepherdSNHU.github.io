...
* Looking at each of the methods in the class, there is a __try/except block__. This code block structure allows for the passing in of data that could cause the application to crash, memory to leak, or nefarious actors to infiltrate. The __try__ section will process the code nested within it. The __if data is not None__ line will pass the value of data only if it meets the criteria of not None. Otherwise, the __else__ statement is run, which will trigger the exception warning. This warning shows an error statement. The __except__ block will only run if the try block is not run. This can be caused by incompatible data being passed into the method when first called. In the event of this happening, an exception is thrown by Python and that error is then assigned to the __e__ variable. Once this exception is thrown, the method is exited and the user will see the error. What this __try/except__ accomplishes is restricting the type of data allowed into the method while also preventing an application crash. When dealing with databases, having security at the forefront of your mind will create a program that protects data, protects the company’s assets and information, while creating a trust between the company and its users. 
